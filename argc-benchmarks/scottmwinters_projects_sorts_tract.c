...
int main() {
  ...
  for (int i = 0; i < SIZE; i++) {
    ...
    __VERIFIER_assume(insertion_list[i] >= -8 && insertion_list[i] <= 8);
    __VERIFIER_assume(selection_list[i] >= -8 && selection_list[i] <= 8);
    __VERIFIER_assume(merge_list[i] >= -8 && merge_list[i] <= 8);
  }

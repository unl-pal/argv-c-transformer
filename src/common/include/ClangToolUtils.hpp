...
/** @brief Returns the major version of the clang on PATH, or nullopt if there isn't one or its
...
inline std::optional<std::string> getResourceDir() {
  ...
  std::optional<std::string> envResourceDir =
      r ? std::optional<std::string>(std::string(r)) : std::nullopt;

  if (envResourceDir) {
    std::string basename = std::filesystem::path(*envResourceDir).filename().string();
    try {
      if (std::stoi(basename) == CLANG_VERSION_MAJOR) return envResourceDir;
  ...
  if (pathVersion) {
    ...
    if (*pathVersion == CLANG_VERSION_MAJOR || !envResourceDir) {
      std::optional<std::string> pathResourceDir =
          readCommandOutput("clang -print-resource-dir 2>/dev/null");
      if (pathResourceDir) {
        if (*pathVersion != CLANG_VERSION_MAJOR)
...
inline bool runFrontendActionCheckingErrors(const std::string &filePath,
  ...
  if (diagnosticsOut) *diagnosticsOut = diagStream.str();
